...
void BPFlow::ReleaseBuffer()
{
	...
	_Release1DBuffer(pDIm1);
	_Release1DBuffer(pDIm2);
	for(int i=0;i<2;i++)
	{
	...
	_Release1DBuffer(pDepthTerm);
	_Release1DBuffer(ptrDepthTerm);
	_Release1DBuffer(pQmsgTerm);
	_Release1DBuffer(ptrQmsgTerm);  
...
void BPFlow::LoadImages(int _width, int _height, int _nchannels, const T_input *pImage1, 
	const T_input *pImage2,const T_input *pDImage1, const T_input *pDImage2, const T_input *pD_RTimage)
{
	Width=_width;
	Height=_height;
	Area=Width*Height;
...
//______________________________________________________________
	_Release1DBuffer(pDIm1);
	_Release1DBuffer(pDIm2);
	pDIm1=new T_input[Width*Height*nChannels];
	pDIm2=new T_input[Width*Height*nChannels];

	memcpy(pDIm1,pDImage1,sizeof(T_input)*Width*Height*nChannels);
	memcpy(pDIm2,pDImage2,sizeof(T_input)*Width*Height*nChannels);
//______________________________________________________________
	_Release1DBuffer(pD_RT);
	pD_RT=new T_input[Width*Height*3];
	memcpy(pD_RT,pD_RTimage,sizeof(T_input)*Width*Height*3);
//______________________________________________________________
    Width2=Width;
    Height2=Height;
}
...
{
	if(x>=0 && int(x)<Width2 && y>=0 && int(y)<Height2)
		return true;
	else
		return false;
}
...
void BPFlow::ComputeDataTerm(PixelBuffer2D<T_message>*& pData, T_message*& ptrData,size_t& TotalMatch,int IDM,double *est_A)
{   
	// allocate the buffer for data term
	TotalMatch=AllocateBuffer<PixelBuffer2D<T_message>,T_message>(pData,ptrData,pWinSize[0],pWinSize[1]);
	double HistMin,HistMax,HistInterval;
		...
		for(ptrdiff_t j=0;j<Width;j++)		// index over x
		{
			...
			for(ptrdiff_t k=-pWinSize[1][index];k<=pWinSize[1][index];k++)  // index over y
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
					...
					case 2:
						for(int n=0;n<nChannels;n++)
						foo+=fabs(pDIm1[index*nChannels+n]-pDIm2[index2*nChannels+n]); // L1 norm
						break;
					case 3:
						T_input xyz[3]={ pD_RT[index2*3],pD_RT[index2*3+1],pD_RT[index*3+2] };
						T_input newxyz[3];
						newxyz[0]=est_A[0]*xyz[0]+est_A[1]*xyz[1]+est_A[2]*xyz[2];
						newxyz[1]=est_A[3]*xyz[0]+est_A[4]*xyz[1]+est_A[5]*xyz[2];
						newxyz[2]=est_A[6]*xyz[0]+est_A[7]*xyz[1]+est_A[8]*xyz[2];

						newxyz[0]=newxyz[0]/newxyz[2]*(Width2/268); //*(Width2/592)
						newxyz[1]=newxyz[1]/newxyz[2]*(Width2/268);//*(Width2/592)
						
						if(!InsideImage(newxyz[0],newxyz[1]))
						continue;
						index2=int(newxyz[1])*Width2+int(newxyz[0]);
						//if (index2!=index)
						//	system("pause");
						for(int n=0;n<nChannels;n++)
						foo+=fabs(pIm1[index*nChannels+n]-pIm1[index2*nChannels+n]); // L1 norm
						break;
					}

					
					pData[index][(k+pWinSize[1][index])*XWinLength+l+pWinSize[0][index]]=foo;
		...
		for(ptrdiff_t j=0;j<Width;j++)		// index over x
		{
			...
			for(ptrdiff_t k=-pWinSize[1][index];k<=pWinSize[1][index];k++)  // index over y
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
					...
					if(!InsideImage(x,y))
						continue;
					int foo=__min(pData[index][(k+pWinSize[1][index])*XWinLength+l+pWinSize[0][index]]/HistInterval,nBins-1);
		...
		for(ptrdiff_t j=0;j<Width;j++)		// index over x
		{
			...
			for(ptrdiff_t k=-pWinSize[1][index];k<=pWinSize[1][index];k++)  // index over y
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
					...
					// if the point is outside the image boundary then continue
					if(!InsideImage(x,y))
						pData[index][_ptr]=DefaultMatchingScore;
                     else if (IsDataTermTruncated) // put truncaitons to the data term
                        pData[index][_ptr]=__min(pData[index][_ptr],DefaultMatchingScore);
				}
		}
...
void BPFlow::UpdateSpatialMessage(int x, int y, int plane, int direction)
{
	...
	double Min=CStochastic::Min(nStates,message_org)+d;
...
void BPFlow::UpdateDualMessage(int x, int y, int plane)
{
	...
	if(plane==0) // from vx plane to vy plane
		for(size_t l=0;l<nStates1;l++){
			Min=message_org[0]+pDataTerm[offset].data()[l*nStates1]+pDepthTerm[offset].data()[l*nStates1]+pQmsgTerm[offset].data()[l*nStates1];//+pQmsgTerm[offset].data()[l*nStates1]
			for(size_t h=0;h<nStates;h++)
				Min=__min(Min,message_org[h]+pDataTerm[offset].data()[l*nStates1+h]+pDepthTerm[offset].data()[l*nStates1+h]+pQmsgTerm[offset].data()[l*nStates1+h]);//+pQmsgTerm[offset].data()[l*nStates1+h]
			message[l]=Min;		
		}
		//	message[l]=CStochastic::Min(nStates,pDataTerm[offset].data()+l*nStates,message_org); // 待修改 
	else					// from vy plane to vx plane
		for(size_t l=0;l<nStates1;l++)
		{
			Min=message_org[0]+pDataTerm[offset].data()[l]+pDepthTerm[offset].data()[l]+pQmsgTerm[offset].data()[l];//+pQmsgTerm[offset].data()[l]
			for(size_t h=0;h<nStates;h++)
				Min=__min(Min,message_org[h]+pDataTerm[offset].data()[h*nStates1+l]+pDepthTerm[offset].data()[h*nStates1+l]+pQmsgTerm[offset].data()[h*nStates1+l]);//+pQmsgTerm[offset].data()[h*nStates1+l]
			message[l]=Min;
		}
				...
				if (k==0)
					...
					for(int l=0;l<nStates;l++)
					{
						if(plane==0) // if the current is horizontal plane
							belief[l]+=pDataTerm[offset].data()[pX[offset*2+1]*nStates+l]+pDepthTerm[offset].data()[pX[offset*2+1]*nStates+l]+pQmsgTerm[offset].data()[pX[offset*2+1]*nStates+l];//+pQmsgTerm[offset].data()[pX[offset*2+1]*nStates+l]
						else   // if the current is vertical plane
						{
							int nStates1=pWinSize[1-plane][offset]*2+1;
							belief[l]+=pDataTerm[offset].data()[l*nStates1+pX[offset*2]]+pDepthTerm[offset].data()[l*nStates1+pX[offset*2]]+pQmsgTerm[offset].data()[l*nStates1+pX[offset*2]];//+pQmsgTerm[offset].data()[l*nStates1+pX[offset*2]]
						}
					}
		...
		for(size_t j=0;j<Width;j++)
		{
			...
			for(size_t k=0;k<2;k++)
			{
				...
				if(i<Height-1)
				{
					...
					//s=m_s;
					//d=m_d;
					energy+=__min((double)abs(pX[offset*2+k]-pWinSize[k][offset]+pOffset[k][offset]-pX[(offset+Width)*2+k]+pWinSize[k][offset+Width]-pOffset[k][offset+Width])*s,d); //  注意此行！！！！
				}
			}
			int vx=pX[offset*2];
			int vy=pX[offset*2+1];
			int nStates=pWinSize[0][offset]*2+1;
			energy+=(pDataTerm[offset].data()[vy*nStates+vx]);   // 注意此行 ！！！ +pDepthTerm[offset].data()[vy*nStates+vx]+pQmsgTerm[offset].data()[vy*nStates+vx]
			for(size_t k=0;k<2;k++)
				energy+=pRangeTerm[k][offset].data()[pX[offset*2+k]]; // 注意此行 ！！！
		}
	return energy;
}
...
void BPFlow::generateCoarserLevel(BPFlow &bp)
{
	...
	bp.D_nTotalMatches=bp.AllocateBuffer(bp.pDepthTerm,bp.ptrDepthTerm,bp.pWinSize[0],bp.pWinSize[1]);
	bp.M_nTotalMatches=bp.AllocateBuffer(bp.pQmsgTerm,bp.ptrQmsgTerm,bp.pWinSize[0],bp.pWinSize[1]);
			...
			for(int ii=0;ii<2;ii++)
				for(int jj=0;jj<2;jj++)
				{
					...
					if(y<Height && x<Width)
					{
						...
						bp.pDepthTerm[offset].data()[k]+=pDepthTerm[y*Width+x].data()[k];
						bp.pQmsgTerm[offset].data()[k]+=pQmsgTerm[y*Width+x].data()[k];
						}
