...
typedef double T_message;  // T_message is the data type of the messages and beliefs
...
private:
	...
	PixelBuffer1D<T_message>* pSpatialMessageback[2];      // the spatial message back
	PixelBuffer1D<T_message>* pDualMessage[2];            // the dual message between two layers
	PixelBuffer1D<T_message>* pDualMessageback[2];            // the dual message back
	...
	T_message* ptrSpatialMessageback[2];
	T_message *ptrDualMessageback[2];
...
public:
	...
	size_t D_nTotalMatches;
	size_t M_nTotalMatches;
	// the buffers for belief propagation
	PixelBuffer2D<T_message>* pDepthTerm;					// the depth term
	PixelBuffer2D<T_message>* pDataTerm;					// the data term
	PixelBuffer2D<T_message>* pQmsgTerm;					// the Qmsg term
	T_message *ptrDataTerm;
	T_message *ptrDepthTerm;
	T_message *ptrQmsgTerm;
	...
	void LoadImages(int _width,int _height,int _nchannels,const T_input* pImage1,const T_input* pImage2,
		const T_input* pDImage1,const T_input* pDImage2,const T_input *pD_RTimage);
	...
	void ComputeDataTerm(PixelBuffer2D<T_message>*& pData, T_message*& ptrData,size_t& TotalMatch,int IDM=1,double* est_A=NULL);
