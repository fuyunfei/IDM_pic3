			...
			for(ptrdiff_t k=-pWinSize[1][index];k<=pWinSize[1][index];k++)  // index over y
				for(ptrdiff_t l=-pWinSize[0][index];l<=pWinSize[0][index];l++)  // index over x
				{
					...
					case 3:
						...
						//newxyz[0]=newxyz[0]/newxyz[2]*(Width2/592); //*(Width2/592)
						//newxyz[1]=newxyz[1]/newxyz[2]*(Width2/592);//*(Width2/592)
...
void BPFlow::UpdateDualMessage(int x, int y, int plane)
{
	...
	if(plane==0) // from vx plane to vy plane
		for(size_t l=0;l<nStates1;l++){
			Min=message_org[0]+pDataTerm[offset].data()[l*nStates1]+pDepthTerm[offset].data()[l*nStates1];//+pQmsgTerm[offset].data()[l*nStates1]
			for(size_t h=0;h<nStates;h++)
				Min=__min(Min,message_org[h]+pDataTerm[offset].data()[l*nStates1+h]+pDepthTerm[offset].data()[l*nStates1+h]);//+pQmsgTerm[offset].data()[l*nStates1+h]
			message[l]=Min;		
		}
		...
		for(size_t l=0;l<nStates1;l++)
		{
			Min=message_org[0]+pDataTerm[offset].data()[l]+pDepthTerm[offset].data()[l];//+pQmsgTerm[offset].data()[l]
			for(size_t h=0;h<nStates;h++)
				Min=__min(Min,message_org[h]+pDataTerm[offset].data()[h*nStates1+l]+pDepthTerm[offset].data()[h*nStates1+l]);//+pQmsgTerm[offset].data()[h*nStates1+l]
			message[l]=Min;
		}
				...
				if (k==0)
					...
					for(int l=0;l<nStates;l++)
					{
						if(plane==0) // if the current is horizontal plane
							belief[l]+=pDataTerm[offset].data()[pX[offset*2+1]*nStates+l]+pDepthTerm[offset].data()[pX[offset*2+1]*nStates+l];//+pQmsgTerm[offset].data()[pX[offset*2+1]*nStates+l]
						else   // if the current is vertical plane
						{
							int nStates1=pWinSize[1-plane][offset]*2+1;
							belief[l]+=pDataTerm[offset].data()[l*nStates1+pX[offset*2]]+pDepthTerm[offset].data()[l*nStates1+pX[offset*2]];//+pQmsgTerm[offset].data()[l*nStates1+pX[offset*2]]
						}
					}
