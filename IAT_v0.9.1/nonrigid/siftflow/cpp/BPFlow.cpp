...
void BPFlow::LoadImages(int _width, int _height, int _nchannels, const T_input *pImage1, 
	const T_input *pImage2,const T_input *pDImage1, const T_input *pDImage2)
{
	Width=_width;
	Height=_height;
	Area=Width*Height;
...
//______________________________________________________________
	...
	//_Release1DBuffer(pDIm2);
	//pDIm1=new T_input[Width*Height*nChannels];
	//pDIm2=new T_input[Width2*Height2*nChannels];

	//memcpy(pDIm1,pDImage1,sizeof(T_input)*Width*Height*nChannels);
	//memcpy(pDIm2,pDImage2,sizeof(T_input)*Width2*Height2*nChannels);
...
void BPFlow::setHomogeneousMRF(int winSize)
{
	...
	for(int i=0;i<2;i++)
		for(int j=0;j<Area;j++)
			//size_t temptest = CStochastic::UniformSampling(5);
...
void BPFlow::ComputeDataTerm(PixelBuffer2D<T_message>*& pData, T_message*& ptrData,size_t& TotalMatch,int whichTerm)
{   
    T_input *img1,*img2;
 	switch (whichTerm){
...
void BPFlow::UpdateDualMessage(int x, int y, int plane)
{
	...
	if(plane==0) // from vx plane to vy plane
		for(size_t l=0;l<nStates1;l++){
			Min=message_org[0]+pDataTerm[offset].data()[l*nStates1]+pDepthTerm[offset].data()[l*nStates1];
			for(size_t h=0;h<nStates;h++)
				Min=__min(Min,message_org[h]+pDataTerm[offset].data()[l*nStates1+h]+pDepthTerm[offset].data()[l*nStates1+h]);
			message[l]=Min;		
		}
	else					// from vy plane to vx plane
		for(size_t l=0;l<nStates1;l++)
		{
			Min=message_org[0]+pDataTerm[offset].data()[l]+pDepthTerm[offset].data()[l];
			for(size_t h=0;h<nStates;h++)
				Min=__min(Min,message_org[h]+pDataTerm[offset].data()[h*nStates1+l]+pDepthTerm[offset].data()[h*nStates1+l]);
			message[l]=Min;
		}
				...
				if (k==0)
					...
					for(int l=0;l<nStates;l++)
					{
						if(plane==0) // if the current is horizontal plane
							belief[l]+=pDataTerm[offset].data()[pX[offset*2+1]*nStates+l]+pDepthTerm[offset].data()[pX[offset*2+1]*nStates+l];
						else   // if the current is vertical plane
						{
							int nStates1=pWinSize[1-plane][offset]*2+1;
							belief[l]+=pDataTerm[offset].data()[l*nStates1+pX[offset*2]]+pDepthTerm[offset].data()[l*nStates1+pX[offset*2]];
						}
					}
...
double BPFlow::GetEnergy()
{
	...
	for(size_t i=0;i<Height;i++)
		for(size_t j=0;j<Width;j++)
		{
			...
			energy+=pDataTerm[offset].data()[vy*nStates+vx];   // 注意此行 ！！！
			for(size_t k=0;k<2;k++)
