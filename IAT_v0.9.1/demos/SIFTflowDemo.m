...
if gridspacing==1
    ...
    depth1reduce8=depth1(patchsize/2+1:end-patchsize/2,patchsize/2+1:end-patchsize/2,:);
else
...
[warpImage1, support] = iat_pixel_warping(Im2,vx,vy);
[depth2reduce8, support] = iat_pixel_warping(depth1reduce8,vx,vy);
save('depth2reduce8','depth2reduce8');
figure;imshow(Im1);title('Image 1');
figure;imshow(uint8(warpImage1));title('WarpImage1');

% visualize alignment error
[~, grayerror] = iat_error2gray(Im1,warpImage1,support);
